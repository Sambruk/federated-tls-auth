...
void http_get(const auth::server_end_point& server,
    ...
    if (curl) {
        ...
        auto full_url = server.url + "/" + suffix_path;
        ...
        auto combined_pins = auth::concatenate_keys(server.pins);
...
int main(int argc, char** argv) {
    ...
    std::string metadata_path, entity, server, cert, key, path;
    ...
    desc.add_options()
        ...
        ("server", po::value<std::string>(&server), "server to connect to")
    ...
    try {
        ...
        auto connection_info =
            auth::load_from_metadata(metadata_path, entity, server);
